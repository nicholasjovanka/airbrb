...
const ReviewPagination = ({ reviewArray, setReviewArray }) => {
  ...
  useEffect(() => {
    setPaginationObj({
      numberOfPage: Math.ceil(reviewArray.length / 6),
      reviewArray,
      currentPage: 1
    })
  }, [reviewArray])
  ...
  const getPages = (page) => {
    ...
    const sliceEndIndex = page === paginationObj.numberOfPage ? paginationObj.reviewArray.length : (page * 6)
    const dataToDisplay = paginationObj.reviewArray.slice(sliceStartIndex, sliceEndIndex);
    setSlicedReviews(dataToDisplay);
  }
  ...
  return (
    <React.Fragment>
      <Box sx={{ flexGrow: 2, display: 'flex', flexDirection: 'column', gap: 2, px: 1, mb: 2 }}>
          {reviewArray.length === 0 && <Typography variant='h6' sx={{ mx: 'auto' }}> No Review Found For this Listing </Typography>}
          {reviewArray.length > 0 && slicedReviews.map((review, index) => (
